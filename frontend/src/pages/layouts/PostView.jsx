...
const Post = ({ header, body, isVolunteer, isFundraiser, imgSrc, username, showVolunteer, numVolunteers: initialNumVolunteers, numDonors: initialNumDonors, onDelete, listVolunteers }) => {
    ...
    const [numVolunteers, setNumVolunteers] = useState(initialNumVolunteers);
    const [numDonors, setNumDonors] = useState(initialNumDonors);
    ...
    useEffect(() => {
        if (clickedDonate) {
            ...
            .then((res) => {
                ...
                setDonor(true);
                setNumDonors(prevNum => prevNum + 1);
                Navigate("/donate");
            })
            .catch((error) => {
    ...
    useEffect(() => {
        if (clickedVolunteer) {
            ...
            .then((res) => {
                ...
                setVolunteer(true);
                setNumVolunteers(prevNum => prevNum + 1);
            })
            .catch((error) => {
                console.log(error)
                            ...
                            <div className="min-h-[100%] flex flex-col gap-y-[10px] justify-end items-end">
                                {isFundraiser && (
                                    <>
                                        <p>{numDonors}</p>
                                        {!donor ? (
                                            <button onClick={() => setClickedDonate(true)} className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-1 px-4 rounded-md">
                                                Donate!
                                            </button>
                                        ) : (
                                ...
                                {isVolunteer && (
                                    <>
                                        <p>{numVolunteers}</p>
                                        {!volunteer ? (
                                            <button onClick={() => setClickedVolunteer(true)} className="bg-green-500 hover:bg-green-600 text-white font-semibold py-1 px-4 rounded-md">
                                                Volunteer!
                                            </button>
                                        ) : (
