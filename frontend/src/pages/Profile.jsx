...
const Profile = () => {
    ...
    useEffect(() => {
        axios
            .get(
                "https://planetpulse.tennisbowling.com/api/projects/get_user_posts",
                { withCredentials: true },
            )
            .then((res) => {
                setPosts(res.data);
    ...
    useEffect(() => {
        axios
            .get(
                "https://planetpulse.tennisbowling.com/api/projects/get_user_social_posts",
                { withCredentials: true },
            )
            .then((res) => {
    ...
    const handleDeletePost = (post_title) => {
        axios
            .post(
                `https://planetpulse.tennisbowling.com/api/projects/delete_post`,
            ...
            .catch((error) => {
                enqueueSnackbar("Error deleting post", {
                    variant: "error",
                    autoHideDuration: 1000,
                });
            });
    ...
    const handleDeleteSocialPost = (post_title) => {
        axios
            .post(
                `https://planetpulse.tennisbowling.com/api/projects/delete_social_post`,
                { post_title: post_title },
                { withCredentials: true },
            )
            .then((res) => {
                setSocialPosts(
                    socialPosts.filter((post) => post.title !== post_title),
                );
                enqueueSnackbar("Social Post Deleted", {
                    variant: "success",
                    autoHideDuration: 1000,
                });
            })
            .catch((error) => {
                enqueueSnackbar("Error deleting social post", {
                    variant: "error",
                    autoHideDuration: 1000,
                });
            });
